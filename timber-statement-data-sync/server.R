# Title: Timber Statement Data Sync
...
shinyServer(function(input, output, session) {
  ...
  royalty_table <- reactive({
    ...
    zone1 <- c(54.27, 44.63, 14.47, 8.44, 5, 5, 5, 5)
    zone2 <- c(54.27, 42.21, 14.47, 8.44, 5, 5, 5, 5)
    zone3 <- c(44.62, 32.56, 12.06, 8.44, 5, 5, 5, 5)
  ...
  fees_table <- reactive({
    class <- c(1, 2, 3, 4, 5, 6, 7, 8)
    fees1 <- c(5, 5, 5, 5, 5, 5, 5, 5)
    fees2 <- c(5, 5, 5, 5, 5, 5, 5, 5)
    fees3 <- c(5, 5, 5, 5, 5, 5, 5, 5)
  ...
  logging_records_summarised <- reactive({
    ...
    logging_records_summary <- logging_records_table() %>%
      ...
      mutate(
        ...
        fees = ((total_volume * 5) * 0.09) * (total_volume * 5)
      )
